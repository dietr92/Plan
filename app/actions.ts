...
function assertProject(project: Project) {
  if (!project.id || !project.name.trim() || !project.client.trim())
    throw new Error("Project mist verplichte velden");
  if (!projectStatuses.includes(project.status))
    throw new Error("Ongeldige projectstatus");
  if (!projectTypes.includes(project.type))
    throw new Error("Ongeldig projecttype");
}
